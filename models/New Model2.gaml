...
global {
    ...
    init {
        ...
        the_graph <- as_edge_graph(routes); // Assuming `routes` can be used as a graph
        
        create stops from: shape_file_stops with:[type:read("MODE")]{
			if type="Industrial"{
				color<-#yellow;
			}
		}
...
experiment AT type: gui {
    ...
    output {
        ...
        display chart_display refresh: every(10#cycles) type: 2d {
            chart "People Objectives" type: pie style: exploded size: {1, 0.5} position: {0, 0.5} {
